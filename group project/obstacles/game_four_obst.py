...
class SnakeGameAI:
    ...
    def _place_food(self):
        ...
        self.food = Point(x, y)
        if self.food in self.snake or self.food == self.obstacle1 or self.food == self.obstacle2:
            self._place_food()
    ...
    def place_obstacles(self):
        ...
        if self.obstacle1 in self.snake or self.obstacle1 == self.food:
            self.place_obstacles()
        if self.obstacle2 in self.snake or self.obstacle2 == self.food:
            self.place_obstacles()
        if self.obstacle3 in self.snake or self.obstacle3 == self.food:
            self.place_obstacles()     
        if self.obstacle4 in self.snake or self.obstacle4 == self.food:
            self.place_obstacles()     
        if self.obstacle1 == self.obstacle2 or self.obstacle1 == self.obstacle3 or self.obstacle1 == self.obstacle4:
            self.place_obstacles()
        if self.obstacle2 == self.obstacle3 or self.obstacle2 == self.obstacle4 or self.obstacle3 == self.obstacle4:
            self.place_obstacles()
    ...
    def play_step(self, action):
        ...
        if self.head == self.obstacle1 or self.head == self.obstacle2 or self.head == self.obstacle3 or self.head == self.obstacle4:
            game_over = True
            reward = -10
            return reward, game_over, self.score
    ...
    def is_collision(self, pt=None):
        ...
        #hits obstacle
        if pt in self.obstacle1 or pt in self.obstacle2 or pt in self.obstacle3 or pt in self.obstacle4:
            return True
    ...
    def _update_ui(self):
        ...
        pygame.draw.rect(self.display, GREEN, pygame.Rect(self.obstacle1.x, self.obstacle1.y, BLOCK_SIZE, BLOCK_SIZE))
        pygame.draw.rect(self.display, GREEN, pygame.Rect(self.obstacle2.x, self.obstacle2.y, BLOCK_SIZE, BLOCK_SIZE))
        pygame.draw.rect(self.display, GREEN, pygame.Rect(self.obstacle3.x, self.obstacle3.y, BLOCK_SIZE, BLOCK_SIZE))
        pygame.draw.rect(self.display, GREEN, pygame.Rect(self.obstacle4.x, self.obstacle4.y, BLOCK_SIZE, BLOCK_SIZE))
