...
class MovingObstacle:
    ...
    def possible_move(self):
        if self.direction == Direction.UP:
            self.position = Point(self.position.x, self.position.y - BLOCK_SIZE)
        elif self.direction == Direction.DOWN:
            self.position = Point(self.position.x, self.position.y + BLOCK_SIZE)
        elif self.direction == Direction.LEFT:
            self.position = Point(self.position.x - BLOCK_SIZE, self.position.y)
...
class SnakeGameAI:
    ...
    def _place_obstacle(self):
        while True:
            ...
            if self.obstacle not in self.snake:
                break
    ...
    def _place_special(self):
        while True:
            ...
            if self.special not in self.snake and self.special != self.obstacle:
                break
    ...
    def play_step(self, action):
        ...
        # Collect special element
            ...
            self.special_elements_collected += 1  # Increment special elements count
            self._place_special()
        
        # Collision with moving obstacle
        elif self.head == self.moving_obstacle.position:
            if self.special_active > 0:
                reward = 0  # No penalty with active special effect
            else:
                self.score -= 1
                reward = -3
                self.snake.pop(0)

        # Collision with stationary obstacle
